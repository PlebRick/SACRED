...
import styles from './TaggingQueue.module.css';

export const TaggingQueue = ({ isOpen, onClose, onNotesUpdated }) => {
  ...
  const loadQueue = useCallback(async () => {
    setLoading(true);
    try {
      const result = await taggingService.getQueue(filterBook || undefined, filterType || undefined);
      setQueue(result.queue || []);
      setStats(result.stats || null);
    } catch (err) {
      console.error('Failed to load tagging queue:', err);
    }
    setLoading(false);
  ...
  const handleAnalyzeAll = async () => {
    ...
    setBatchResult(null);
    try {
      const result = await taggingService.batch('analyze');
  ...
  const handleAutoTag = async () => {
    ...
    setBatchResult(null);
    try {
      const result = await taggingService.batch('auto', undefined, 0.6);
      setBatchResult(result);
      await loadQueue();
  ...
  const handleReviewDone = async () => {
    ...
    // The queue will be refreshed; the first item is the next to review
  };
        ...
        <div className={styles.modal} onClick={e => e.stopPropagation()}>
          ...
          <div className={styles.header}>
            <div className={styles.headerLeft}>
              <h3 className={styles.title}>Tagging Queue</h3>
              <span className={styles.subtitle}>
                {stats ? `${stats.untaggedNotes} untagged of ${stats.totalNotes} notes` : 'Loading...'}
              </span>
            </div>
          ...
          {batchResult && (
            <div className={styles.batchResult}>
              <span>
                ...
                {batchResult.tagged > 0 && `, tagged ${batchResult.tagged}`}
          ...
          <div className={styles.queueList}>
            {loading ? (
              ...
              <div className={styles.emptyState}>
                ...
                <div className={styles.emptyTitle}>All notes tagged!</div>
                <div className={styles.emptyDescription}>
                  Every note has a primary topic assigned.
                </div>
              </div>
            ) : (
              queue.map(note => (
