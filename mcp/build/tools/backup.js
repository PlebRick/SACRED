...
import { toApiFormat, topicToApiFormat, inlineTagTypeToApiFormat, } from '../utils/format.js';
...
export function registerBackupTools(server) {
    ...
    server.tool('full_export', 'Export all SACRED data (notes, topics, inline tag types) as JSON backup', {}, async () => {
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error exporting data: ${error}` }],
                isError: true,
            };
        }
    });
    // full_import - Import backup (upsert)
    server.tool('full_import', 'Import SACRED backup data (upserts: updates existing, inserts new)', {
        notes: z
            .array(z.object({
    ...
    }, async ({ notes, topics, inlineTagTypes }) => {
        try {
            // Prepare statements
        ...
        `);
            ...
            const importTransaction = db.transaction(() => {
                    ...
                    for (const tagType of inlineTagTypes) {
                        try {
                            ...
                            if (existing) {
                                ...
                                insertInlineTagTypeStmt.run(tagType.id, tagType.name, tagType.color, tagType.icon ?? null, tagType.isDefault ? 1 : 0, tagType.sortOrder ?? 0, tagType.createdAt ?? now);
                ...
                if (topics && Array.isArray(topics)) {
                    for (const topic of topics) {
                        try {
                            ...
                            if (existing) {
                                updateTopicStmt.run(topic.name, topic.parentId ?? null, topic.sortOrder ?? 0, topic.systematicTagId ?? null, topic.updatedAt ?? now, topic.id);
                                topicsUpdated++;
                            }
                            else {
                                insertTopicStmt.run(topic.id, topic.name, topic.parentId ?? null, topic.sortOrder ?? 0, topic.systematicTagId ?? null, topic.createdAt ?? now, topic.updatedAt ?? now);
                                topicsInserted++;
                            }
                        }
                        catch (e) {
                ...
                for (const note of notes) {
                    try {
                        ...
                        if (existing) {
                            updateNoteStmt.run(note.book.toUpperCase(), note.startChapter, note.startVerse ?? null, note.endChapter, note.endVerse ?? null, note.title ?? '', note.content ?? '', note.type ?? 'note', note.primaryTopicId ?? null, note.updatedAt ?? now, id);
                            notesUpdated++;
                        }
                        else {
                            insertNoteStmt.run(id, note.book.toUpperCase(), note.startChapter, note.startVerse ?? null, note.endChapter, note.endVerse ?? null, note.title ?? '', note.content ?? '', note.type ?? 'note', note.primaryTopicId ?? null, note.createdAt ?? now, note.updatedAt ?? now);
                            notesInserted++;
                        }
                        // Handle tags
            ...
            return {
                content: [{ type: 'text', text: `Error importing data: ${error}` }],
                isError: true,
            };
        }
    });
    // delete_all_notes - Clear all notes (requires confirmation)
    server.tool('delete_all_notes', 'Delete ALL notes from the database. DESTRUCTIVE - requires confirm=true parameter.', {
        ...
        try {
            ...
            const countBefore = db.prepare('SELECT COUNT(*) as count FROM notes').get().count;
            ...
            return {
                content: [{ type: 'text', text: `Error deleting notes: ${error}` }],
                isError: true,
            };
        }
    });
    // get_last_modified - Get timestamp of most recently modified note
    server.tool('get_last_modified', 'Get the timestamp of the most recently modified note', {}, async () => {
        try {
            ...
            return {
                content: [{ type: 'text', text: `Error getting last modified: ${error}` }],
                isError: true,
            };
        }
    });
