...
export function registerResources(server: McpServer): void {
  ...
  server.resource(
    'all-notes',
    'sacred://notes/all',
    { description: 'All Bible study notes' },
    async (uri) => {
  ...
  server.resource(
    'notes-summary',
    'sacred://notes/summary',
    { description: 'Summary statistics of Bible study notes' },
    async (uri) => {
  ...
  server.resource(
    'note-by-id',
    new ResourceTemplate('sacred://notes/{id}', { list: undefined }),
    { description: 'A single Bible study note by ID' },
    async (uri, variables) => {
  ...
  server.resource(
    'notes-by-book',
    new ResourceTemplate('sacred://notes/book/{book}', { list: undefined }),
    { description: 'All notes for a specific Bible book' },
    async (uri, variables) => {
  ...
  server.resource(
    'notes-by-chapter',
    new ResourceTemplate('sacred://notes/chapter/{book}/{chapter}', { list: undefined }),
    { description: 'All notes for a specific Bible book and chapter' },
    async (uri, variables) => {
      const book = String(variables.book).toUpperCase();
      ...
      const notes = db
        ...
        .all(book, chapterNum, chapterNum) as DbNote[];
