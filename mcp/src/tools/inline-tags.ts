...
import { DbInlineTagType, inlineTagTypeToApiFormat, DbInlineTag, inlineTagToApiFormat } from '../utils/format.js';
import { logger } from '../utils/logger.js';

/**
 * Register inline tag tools for MCP
 */
export function registerInlineTagTools(server: McpServer): void {
  // list_inline_tag_types - Get all tag type definitions
  server.tool(
    'list_inline_tag_types',
    'Get all inline tag type definitions (predefined and custom)',
    {},
    async () => {
      try {
        const types = db.prepare('SELECT * FROM inline_tag_types ORDER BY sort_order').all() as DbInlineTagType[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error listing inline tag types: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Create a new custom inline tag type',
    {
    ...
    async ({ name, color, icon }) => {
      try {
        if (!name || !name.trim()) {
          return {
            content: [{ type: 'text' as const, text: 'Tag type name is required' }],
            isError: true,
          };
        }
        ...
        if (!color.match(/^#[0-9a-fA-F]{6}$/)) {
          return {
            content: [{ type: 'text' as const, text: 'Color must be a valid hex code (e.g., "#60a5fa")' }],
            isError: true,
          };
        }
        ...
        if (existing) {
          return {
            content: [{ type: 'text' as const, text: `A tag type with name "${name}" already exists` }],
            isError: true,
          };
        }

        // Get next sort order
        const maxOrder = (db.prepare('SELECT MAX(sort_order) as max FROM inline_tag_types').get() as { max: number | null }).max || 0;
        ...
        const tagType = db.prepare('SELECT * FROM inline_tag_types WHERE id = ?').get(id) as DbInlineTagType;
        ...
        return {
          content: [{ type: 'text' as const, text: `Error creating inline tag type: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Update an existing inline tag type',
    {
    ...
    async ({ id, name, color, icon, sortOrder }) => {
      try {
        const existing = db.prepare('SELECT * FROM inline_tag_types WHERE id = ?').get(id) as DbInlineTagType | undefined;

        if (!existing) {
          return {
            content: [{ type: 'text' as const, text: `Tag type not found with ID: ${id}` }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: 'text' as const, text: `Error updating inline tag type: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Delete a custom inline tag type (cannot delete default types)',
    {
    ...
    async ({ id }) => {
      try {
        ...
        if (existing.is_default === 1) {
          return {
            content: [{ type: 'text' as const, text: 'Cannot delete default tag types' }],
            isError: true,
          };
        }
        ...
        return {
          content: [{ type: 'text' as const, text: `Error deleting inline tag type: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'list_inline_tags',
    'Get inline tags extracted from notes with optional filtering',
    {
    ...
    async ({ tagType, book, search, limit = 100, offset = 0 }) => {
      try {
        ...
        const tags = db.prepare(query).all(...params) as DbInlineTag[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error listing inline tags: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Get inline tag counts grouped by tag type',
    {},
    async () => {
      try {
        const counts = db
        ...
        `
          ...
          .all() as (DbInlineTagType & { count: number })[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error getting inline tag counts: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'search_inline_tags',
    'Search inline tagged content by keyword',
    {
    ...
    async ({ query, limit = 50 }) => {
      try {
        ...
        `
          ...
          .all(`%${query}%`, limit) as DbInlineTag[];

        // Group by tag type
        const byType: Record<string, typeof tags> = {};
              ...
              text: JSON.stringify(
                {
                  ...
                  byType: Object.fromEntries(
                    Object.entries(byType).map(([type, tagList]) => [type, tagList.map(inlineTagToApiFormat)])
                  ),
                  count: tags.length,
                },
                null,
        ...
        return {
          content: [{ type: 'text' as const, text: `Error searching inline tags: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Re-seed the default inline tag types (upserts, does not delete custom types)',
    {},
    async () => {
      try {
        const defaultTypes = [
        ...
        return {
          content: [{ type: 'text' as const, text: `Error seeding inline tag types: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  logger.info(
    'Registered inline tag tools: list_inline_tag_types, create_inline_tag_type, update_inline_tag_type, delete_inline_tag_type, list_inline_tags, get_inline_tags_by_type, search_inline_tags, seed_inline_tag_types'
  );
}
