 ...
 */
function setNoteTags(noteId: string, tagIds: string[]): void {
...
export function registerCrudTools(server: McpServer): void {
    ...
    async ({ book, startChapter, startVerse, endChapter, endVerse, title = '', content = '', type = 'note', primaryTopicId, tags }) => {
      try {
        ...
        if (primaryTopicId) {
          ...
          if (!topic) {
            return {
              content: [{ type: 'text' as const, text: `Primary topic not found: ${primaryTopicId}` }],
              isError: true,
            };
          }
        }
  ...
  server.tool(
    'set_note_topics',
    'Assign primary and/or secondary topics to an existing note',
    {
    ...
    async ({ noteId, primaryTopicId, tags, addTags, removeTags }) => {
      try {
        ...
        if (!note) {
          return {
            content: [{ type: 'text' as const, text: `Note not found with ID: ${noteId}` }],
            isError: true,
          };
        }
        ...
        let primaryTopic: DbTopic | undefined;
        if (updatedNote.primary_topic_id) {
          primaryTopic = db.prepare('SELECT * FROM topics WHERE id = ?').get(updatedNote.primary_topic_id) as DbTopic | undefined;
        }

        const tagTopics = currentTags.length > 0
          ? (db.prepare(`SELECT * FROM topics WHERE id IN (${currentTags.map(() => '?').join(',')})`).all(...currentTags) as DbTopic[])
          : [];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error setting note topics: ${error}` }],
          isError: true,
        };
      }
    }
