...
export function registerBulkTools(server: McpServer): void {
  ...
  server.tool(
    'search_notes',
    'Search for notes by keyword in title and content using full-text search',
    {
    ...
    async ({ query, limit = 20 }) => {
      try {
        ...
        const results = db
          ...
          .all(query, limit) as (DbNote & { rank: number })[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error searching notes: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'export_notes',
    'Export all Bible study notes as JSON (matches SACRED app backup format)',
    {},
    async () => {
      try {
        const notes = db.prepare('SELECT * FROM notes ORDER BY created_at ASC').all() as DbNote[];
  ...
  server.tool(
    'import_notes',
    'Import Bible study notes from JSON (upsert: updates existing notes, inserts new ones)',
    {
      notes: z
        .array(
          z.object({
    ...
    async ({ notes }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error importing notes: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    'Get a summary of all Bible books that have notes, with counts',
    {},
    async () => {
      try {
        const books = db
          .prepare(
            ...
            ORDER BY
              CASE book
                ...
                WHEN 'OBA' THEN 31 WHEN 'JON' THEN 32 WHEN 'MIC' THEN 33 WHEN 'NAH' THEN 34 WHEN 'HAB' THEN 35
          ...
          .all() as { book: string; noteCount: number; firstChapter: number; lastChapter: number }[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error getting books: ${error}` }],
          isError: true,
        };
      }
    }
