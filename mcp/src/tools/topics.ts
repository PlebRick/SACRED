...
import { DbTopic, topicToApiFormat, DbNote, toApiFormat } from '../utils/format.js';
import { logger } from '../utils/logger.js';

/**
 * Topic with children for tree structure
 */
interface TopicWithChildren extends DbTopic {
  noteCount?: number;
 ...
 */
function buildTree(topics: DbTopic[], parentId: string | null = null): TopicWithChildren[] {
  return topics
 ...
 */
function getDescendantIds(topicId: string): string[] {
 ...
 */
function getTopicNoteCount(topicId: string): number {
 ...
 */
function addNoteCounts(tree: TopicWithChildren[]): TopicWithChildren[] {
 ...
 */
function topicTreeToApiFormat(topic: TopicWithChildren): Record<string, unknown> {
  return {
...
export function registerTopicTools(server: McpServer): void {
  ...
  server.tool(
    'list_topics',
    'Get the topic taxonomy as a tree structure or flat list',
    {
    ...
    async ({ flat = false, includeNoteCounts = true }) => {
      try {
        const topics = db.prepare('SELECT * FROM topics ORDER BY sort_order, name').all() as DbTopic[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error listing topics: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'get_topic',
    'Get a single topic by its ID, including children and note count',
    {
      id: z.string().describe('The UUID of the topic to retrieve'),
    },
    async ({ id }) => {
      try {
        const topic = db.prepare('SELECT * FROM topics WHERE id = ?').get(id) as DbTopic | undefined;

        if (!topic) {
          return {
            content: [{ type: 'text' as const, text: `Topic not found with ID: ${id}` }],
            isError: true,
          };
        }

        // Get children
        const children = db.prepare('SELECT * FROM topics WHERE parent_id = ? ORDER BY sort_order, name').all(id) as DbTopic[];

        // Get parent path
        const getParentPath = (topicId: string | null): DbTopic[] => {
          if (!topicId) return [];
          const parent = db.prepare('SELECT * FROM topics WHERE id = ?').get(topicId) as DbTopic | undefined;
          if (!parent) return [];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error getting topic: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'get_topic_notes',
    'Get all notes that have a specific topic (primary or secondary), including descendant topics',
    {
    ...
    async ({ id, limit = 100, offset = 0 }) => {
      try {
        ...
        `
          ...
          .all(...allIds, ...allIds, limit, offset) as DbNote[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error getting topic notes: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    ...
    async ({ name, parentId, sortOrder = 0, systematicTagId }) => {
      try {
        if (!name || !name.trim()) {
          return {
            content: [{ type: 'text' as const, text: 'Topic name is required' }],
            isError: true,
          };
        }
        ...
        if (parentId) {
          ...
          if (!parent) {
            return {
              content: [{ type: 'text' as const, text: 'Parent topic not found' }],
              isError: true,
            };
          }
        }
        ...
        if (systematicTagId) {
          ...
          if (!tag) {
            return {
              content: [{ type: 'text' as const, text: 'Systematic tag not found' }],
              isError: true,
            };
          }
        }
        ...
        const topic = db.prepare('SELECT * FROM topics WHERE id = ?').get(id) as DbTopic;
        ...
        return {
          content: [{ type: 'text' as const, text: `Error creating topic: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'update_topic',
    'Update an existing topic (partial update - only provided fields are changed)',
    {
    ...
    async ({ id, name, parentId, sortOrder, systematicTagId }) => {
      try {
        const existing = db.prepare('SELECT * FROM topics WHERE id = ?').get(id) as DbTopic | undefined;
        ...
        if (newParentId === id) {
          return {
            content: [{ type: 'text' as const, text: 'Topic cannot be its own parent' }],
            isError: true,
          };
        }
        ...
        if (newParentId) {
          ...
          while (currentParent) {
            if (currentParent === id) {
              return {
                content: [{ type: 'text' as const, text: 'Circular reference detected' }],
                isError: true,
              };
            }
        ...
        return {
          content: [{ type: 'text' as const, text: `Error updating topic: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'delete_topic',
    'Delete a topic by ID (also removes topic from notes and deletes child topics)',
    {
    ...
    async ({ id }) => {
      try {
        ...
        return {
          content: [{ type: 'text' as const, text: `Error deleting topic: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  server.tool(
    'seed_topics',
    'Seed the default topic taxonomy (only works if no topics exist)',
    {},
    async () => {
      try {
        const existingCount = (db.prepare('SELECT COUNT(*) as count FROM topics').get() as { count: number }).count;
        if (existingCount > 0) {
          return {
            content: [
              {
        ...
        const createTopic = (name: string, parentId: string | null = null, sortOrder = 0, systematicTagId: string | null = null): string => {
        ...
        const finalCount = (db.prepare('SELECT COUNT(*) as count FROM topics').get() as { count: number }).count;
        ...
        return {
          content: [{ type: 'text' as const, text: `Error seeding topics: ${error}` }],
          isError: true,
        };
      }
    }
    ...
    async ({ query, limit = 10 }) => {
      try {
        ...
        `
          ...
          .all(`%${query}%`, query, `${query}%`, limit) as DbTopic[];
        ...
        return {
          content: [{ type: 'text' as const, text: `Error searching topics: ${error}` }],
          isError: true,
        };
      }
    }
  ...
  logger.info(
    'Registered topic tools: list_topics, get_topic, get_topic_notes, create_topic, update_topic, delete_topic, seed_topics, find_topic_by_name'
  );
}
